 ...
 * @author ADMIN
 */
public class CellRender_Cruce_Arl extends DefaultTableCellRenderer{
    @Override
        ...
        if (table.getColumnName(column).equals("Cedula")) {
            ...
            if (table.getColumnName(column).equals("F Ingreso (DD-MM-AAAA)")) {
                ...
                if (table.getColumnName(column).equals("Tasa (%)")) {
                    ...
                    if (table.getColumnName(column).equals("Nombre")) {
                        if (value!=null) {
                            if(!value.toString().equals("")){
                                cell.setOpaque(true);
                                cell.setBackground(new Color(150,240,160));
...
//    @Override
//    public void setHorizontalAlignment(int t){
...
public boolean check_fecha(Object fecha){
    ...
    if (fecha!=null) {
        if (chech_char(fecha.toString().trim(),"'#$%&()=?¡¿*+[]{};:<>,.")) {
            if (!fecha.toString().trim().equals("") & count_char(fecha.toString().trim(),'-')==2 & (fecha.toString().length()==10 | fecha.toString().length()==9)) {
                int str_año=0;
                int str_mes=0;
                int str_dia=0;
                try {
...
try{
    float num = Float.parseFloat(cadena.replace(',', '.'));
}catch (Exception e){
    //e.printStackTrace();
    return false;
}
